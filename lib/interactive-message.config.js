...
plugin.keyboard.config.autoDelayMs = 1;
...
const octokit = new plugin.octokit.Octokit();
const fetchMyIssues = () =>
  octokit.issues
    .listForRepo({
...
{{body}}`,
  questionDictionary: [
    ...
    },
    {
      ...
      exInitValue: (p) => plugin.keyboard.type(p),
      ...(() =>
  ...
  config: {
     ...
     */
    color:
      process.env.DEEPL_AUTH_KEY && process.env.GRAMMAR_API_KEY
        ? "green"
        : "yellow",
    errorColor: "red",
